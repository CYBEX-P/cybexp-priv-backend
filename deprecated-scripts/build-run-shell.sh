...
IMAGE_NAME=cybexp-priv-backend-api
...
if [ "$DOCKER_STATE" = "inactive" ]; then
   ...
   sudo docker build -t $IMAGE_NAME $DOCKERFILE_LOC && sudo docker run -p 5001:5000 -v $FULL_PATH/secrets:/secrets/ --entrypoint /bin/bash -it $IMAGE_NAME
